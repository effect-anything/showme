...
TARGET_DIR="$REPO_ROOT/.references"

print_usage() {
  cat <<'EOF'
Usage: .references/clone-reference-repos.sh [--pull] [--depth N] [--full]

Clones a set of reference repositories into .references/.
- Existing repos: always fetch; optionally pull (ff-only).
- Default clone mode: --depth 1.

Examples:
  .references/clone-reference-repos.sh
  .references/clone-reference-repos.sh --pull
  .references/clone-reference-repos.sh --full
  .references/clone-reference-repos.sh --depth 50
EOF
}
...
clone_or_update "https://github.com/Effect-TS/effect-smol" "effect"
